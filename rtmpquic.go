...
func pullrtmp(url, filename string, protocol string) {
	...
	logQoEEvent(qoeEvent{role: "puller", event: "stream_end", physicalTimeMS: qoeNowMS(), note: qoeNote("err=%v", err)})
...
func pushrtmp(url, filename string, protocol string) {
	...
	logQoEEvent(qoeEvent{role: "pusher", event: "pusher_start", physicalTimeMS: qoeNowMS(), note: qoeNote("protocol=%s", protocol)})
...
func loopPush(tags []httpflv.Tag, session *rtmp.PushSession) {
	...
	for _, tag := range tags {
		...
		if tag.IsMetadata() {
			if totalBaseTS == 0 {
				...
				physicalMS := qoeNowMS()
				if !loggedFirstTagSend {
					logQoEEvent(qoeEvent{
						role:           "pusher",
						event:          "first_tag_send",
						flvTimestampMS: qoeUint32(h.TimestampAbs),
						physicalTimeMS: physicalMS,
						tagType:        qoeFLVTagType(tag),
						frameType:      qoeFLVFrameType(tag),
						chunkSize:      qoeInt(len(chunks)),
						streamID:       qoeInt(h.MsgStreamID),
						note:           "tag_index=metadata",
					})
					loggedFirstTagSend = true
				}
		...
		if !loggedFirstTagSend {
			logQoEEvent(qoeEvent{
				...
				chunkSize:      qoeInt(len(chunks)),
				streamID:       qoeInt(h.MsgStreamID),
				note:           qoeNote("tag_index=%d", i),
			})
			loggedFirstTagSend = true
		}
		if tag.Header.Type == httpflv.TagTypeVideo {
			videoTagCount++
			if qoeShouldLogVideo(videoTagCount) {
				logQoEEvent(qoeEvent{
					role:           "pusher",
					event:          "video_tag_send",
					flvTimestampMS: qoeUint32(h.TimestampAbs),
					physicalTimeMS: physicalMS,
					tagType:        qoeFLVTagType(tag),
					frameType:      qoeFLVFrameType(tag),
					chunkSize:      qoeInt(len(chunks)),
					streamID:       qoeInt(h.MsgStreamID),
					note:           qoeNote("tag_index=%d video_index=%d", i, videoTagCount),
				})
			}
		} else if tag.Header.Type == httpflv.TagTypeAudio && qoeShouldLogAudio() {
			logQoEEvent(qoeEvent{
				role:           "pusher",
				event:          "audio_tag_send",
				flvTimestampMS: qoeUint32(h.TimestampAbs),
				physicalTimeMS: physicalMS,
				tagType:        qoeFLVTagType(tag),
				frameType:      qoeFLVFrameType(tag),
		...
		if err := session.Write(chunks); err != nil {
			logQoEEvent(qoeEvent{
				...
				event:          "stream_end",
				flvTimestampMS: qoeUint32(h.TimestampAbs),
				physicalTimeMS: qoeNowMS(),
				tagType:        qoeFLVTagType(tag),
				frameType:      qoeFLVFrameType(tag),
				chunkSize:      qoeInt(len(chunks)),
				streamID:       qoeInt(h.MsgStreamID),
				note:           qoeNote("write_error=%v", err),
			})
	...
	logQoEEvent(qoeEvent{role: "pusher", event: "stream_end", physicalTimeMS: qoeNowMS(), note: qoeNote("tags=%d", len(tags))})
