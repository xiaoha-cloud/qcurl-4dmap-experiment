...
Primary metric: receiver-side ``path :* mean tp`` from pull logs (sum over paths).
Fallback: pcap/tshark via pcap_throughput.py when --use-pcap and pcaps exist.

Windows (seconds from first mean-tp sample in pull log):
  0-50, 50-60, 50-100, 100-110, 100-150

Improvement:
  (enhanced_mean - baseline_mean) / baseline_mean * 100

Usage
-----
    python3 scripts/analyze/fig7_throughput_compare.py --out derived/fig7_compare \\
        -r baseline:logs_exp/session_X/fig7_um_baseline \\
        -r T:logs_exp/session_X/fig7_um_T \\
        -r learn:logs_exp/session_X/fig7_um_learn
"""
