...
def _per_second_retrans_delta(run_dir: Path, method: str) -> pd.DataFrame:
    ...
    result = (
        path_b.groupby("time_s", as_index=False)
        .agg(
            path_b_retrans_bytes_delta_sum=("retrans_delta", "sum"),
            n_samples=("retrans_delta", "count"),
        )
        .sort_values("time_s")
...
def _per_second_loss_monitor(mon: pd.DataFrame, method: str) -> pd.DataFrame:
    ...
    result = (
        m.groupby("time_s", as_index=False)
        .agg(
            path_b_monitor_loss_mean=("loss", "mean"),
            path_b_monitor_loss_p95=("loss", lambda series: series.quantile(0.95)),
            n_samples=("loss", "count"),
        )
        .sort_values("time_s")
        .reset_index(drop=True)
    )
