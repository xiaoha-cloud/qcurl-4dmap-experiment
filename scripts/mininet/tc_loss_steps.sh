#!/usr/bin/env bash
# Phase 2 (loss-only): fixed delay + step netem loss on one interface (path B by default).
# Timeline is relative to when this script starts (before server/pull/push in --run-exp).
# Profile format: see loss_profile.example.env
set -euo pipefail
...
apply_loss() {
  local loss="$1"
  if tc qdisc replace dev "$IFACE" root netem delay "${FIXED_DELAY_MS}ms" loss "${loss}%" 2>/dev/null; then
    return 0
  fi
  log "replace failed, trying del+add (may drop Mininet TCLink shaping on this iface)"
  tc qdisc del dev "$IFACE" root 2>/dev/null || true
  tc qdisc add dev "$IFACE" root netem delay "${FIXED_DELAY_MS}ms" loss "${loss}%"
}
...
for i in "${!T_AT[@]}"; do
  ...
  log "step $((i + 1))/${#T_AT[@]} at=${at}s loss=${loss}% delay=${FIXED_DELAY_MS}ms dev=${IFACE}"
  apply_loss "$loss"
  prev=$at
done
