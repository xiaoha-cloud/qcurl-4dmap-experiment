...
if [[ "$EXPERIMENT_FAMILY" == "clean_controlled" ]]; then
  ...
  elif [[ "$PROFILE_KIND" == "loss" ]]; then
    [[ "${TC_LOSS_FIXED_BW_MBIT:-}" == "20" ]] || { echo "[error] clean loss requires TC_LOSS_FIXED_BW_MBIT=20" >&2; exit 2; }
    [[ "${TC_LOSS_FIXED_DELAY_MS:-}" == "40" ]] || { echo "[error] clean loss requires TC_LOSS_FIXED_DELAY_MS=40" >&2; exit 2; }
  fi
fi
...
check_clean_configuration_only() {
  ...
  if [[ "$PROFILE_KIND" == "bandwidth" ]]; then
    ...
    echo "[configuration] qdisc_hierarchy=root_tbf_1_to_child_netem_10 fixed_bw_mbit=$TC_LOSS_FIXED_BW_MBIT fixed_delay_ms=$TC_LOSS_FIXED_DELAY_MS dynamic_loss_profile=metadata_driven"
  fi
  validate_profile
