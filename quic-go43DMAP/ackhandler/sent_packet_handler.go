...
func (h *sentPacketHandler) SentPacket(packet *Packet) error {
	...
	h.totalrecvsize += float64(packet.Length)
	h.thistimesize += float64(packet.Length)
	...
	//utils.Infof("last setn :%v,",packet.PacketNumber)
	...
	if isRetransmittable {
		//utils.Infof("[ach]retransmittalbe")
		packet.SendTime = now
		h.bytesInFlight += packet.Length
		h.packetHistory.PushBack(*packet)
		h.numNonRetransmittablePackets = 0
	} else {
		//utils.Infof("[ach]in-retransmit!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1")
...
func (h *sentPacketHandler) ReceivedClosePath(f *wire.ClosePathFrame, withPacketNumber protocol.PacketNumber, rcvTime time.Time) error {
	if f.LargestAcked > h.lastSentPacketNumber {
		//utils.Infof("last setn :%v, lagest ack:%v",h.lastSentPacketNumber,  f.LargestAcked)
		return errAckForUnsentPacket
	}
...
func (h *sentPacketHandler) SendingAllowed(flag bool) bool {
	congestionLimited := h.bytesInFlight > h.congestion.GetCongestionWindow()
	maxTrackedLimited := protocol.PacketNumber(len(h.retransmissionQueue)+h.packetHistory.Len()) >= protocol.MaxTrackedSentPackets
	//utils.Debugf("cwnd:%d",h.congestion.GetCongestionWindow())
	// if (flag){
	// 	return true
	// }
