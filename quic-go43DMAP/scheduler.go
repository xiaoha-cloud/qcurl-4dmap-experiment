...
func (m *monitor) monitorApplyUtility(pth *path) {
	...
	pm := PathMetrics{
		...
		BWbps:             float64(m.state_bw[pth.pathID]) * 8,
		OWDms:             float64(owdNs) / 1e6,
		RTTLatestMs:       float64(pth.rttStats.LatestRTT()) / float64(time.Millisecond),
		RTTSmoothedMs:     float64(pth.rttStats.SmoothedRTT()) / float64(time.Millisecond),
		RTTMinMs:          float64(pth.rttStats.MinRTT()) / float64(time.Millisecond),
